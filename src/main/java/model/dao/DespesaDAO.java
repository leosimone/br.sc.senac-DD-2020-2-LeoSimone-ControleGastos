...
public class DespesaDAO {
	...
	public int cadastrarDespesaDAO(DespesaVO despesaVO) {
		...
		try {
			...
			System.out.println("Erro ao executar query de cadastro de Despesa.");
	...
	public int excluirDespesaDAO(DespesaVO despesaVO) {
		...
		try {
			...
			System.out.println("Erro ao executar a query que exclui a despesa.");
			System.out.println("Erro: " + e.getMessage());
		}finally {
	...
	public int atualizarDespesaDAO(DespesaVO despesaVO) {
		...
		String query = "UPDARE despesa SET "
		...
		try {
			...
			System.out.println("Erro ao executar query de atualização de Despesa.");
			System.out.println("Erro: " + e.getMessage());
		} finally {
			Banco.closeStatement(stmt);
			Banco.closeConnection(conn);
		...
		try {
			resultado = stmt.executeQuery(query);
				while(resultado.next()){
					...
					despesa.setCategoria(resultado.getNString(7));
					listaDespesasVO.add(despesa);
					
				}
					
		} catch (SQLException e) {
			System.out.println("Erro ao executar a query de lista de Despesas de um Usuário.");
			System.out.println("Erro: " + e.getMessage());
		}finally {
	...
	public DespesaVO consultarDespesaDAO(DespesaVO despesaVO) {
		...
		try {
			...
			System.out.println("Erro ao executar a query de consulta de uma Despesa de um Usuário.");
			System.out.println("Erro: " + e.getMessage());
		}finally {
			Banco.closeResultSet(resultado);
			Banco.closeStatement(stmt);
